...
const getOrderById = async (req, res) => {
  try {
    ...
    if (!isAdmin && !isSeller && order.user._id.toString() !== userId.toString()) {
  ...
  try {
    ...
    if (shippingAddress) {
      ...
      if (isAdmin || isSeller || (isOwner && order.status === "pending")) {
        ...
        return res.status(403).json({ message: "Cannot update address for this order" });
      }
    }
...
module.exports = { createOrder, listOrders, getOrderById, updateOrder, deleteOrder };