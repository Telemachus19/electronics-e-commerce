...
const defaultRoles = [
  ...
  },
  {
    ...
    permissions: ['products:read', 'orders:create', 'orders:read:own', 'profile:update:own'],
    isSystem: true
  },
  {
    name: 'seller',
    description: 'Can manage own catalog and orders for own products',
    permissions: ['products:create:own', 'products:update:own', 'orders:read:seller'],
    isSystem: true
  }
];
...
const seedRoles = async () => {
  try {
    ...
    console.log('Roles seeded successfully:', roles.map((role) => role.name).join(', '));
  } catch (error) {
